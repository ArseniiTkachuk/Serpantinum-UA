...
EOF
  ...
  local OSC8_TG=$'\e]8;;https://t.me/tkachukarsen'
    ...
    while true; do
      ...
      if [ "$MULTIPLE_COMPOSITORS_DETECTED" = true ]; then
        if [ ${#SELECTED_COMPOSITORS[@]} -eq 0 ]; then
          ...
          COMP_MENU_ITEM="${C_GREEN}$(t "installer.ui.target_compositor_selected" "comps=$(
            IFS=,
            echo "${SELECTED_COMPOSITORS[*]}"
          )")${RESET}"
        fi
        ...
        if [ ${#SELECTED_COMPOSITORS[@]} -eq 0 ]; then
          ...
          COMP_MENU_ITEM="$(t "installer.ui.target_compositor_label" "comps=$(
            IFS=,
            echo "${SELECTED_COMPOSITORS[*]}"
          )")"
        fi
